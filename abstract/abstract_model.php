...
abstract class abstract_model{
	...
	private $sClassSgbd=null;
	private $oSgbd=null;
	private static $tInstance=array();

	protected $tAssoc=array();
	...
	public function getSgbd(){
		$bFirst=false;
		if($this->sClassSgbd==null){
			$sVarIniConfig=_root::getConfigVar('model.ini.var','db');
			$tClassSgbd=_root::getConfigVar($sVarIniConfig);
			if(!$tClassSgbd){
				throw new Exception('Il vous manque un fichier de configuration ou le bloc de configuration ['.$sVarIniConfig.'] concernant la connexion
			...
			$this->sClassSgbd='sgbd_'.$tClassSgbd[$this->sConfig.'.sgbd'];
			$bFirst=true;
			if(substr($this->sClassSgbd,0,8)=='sgbd_pdo'){
				$sClassPath=_root::getConfigVar('path.lib').'sgbd/pdo/'.$this->sClassSgbd.'.php';
			}elseif(substr($this->sClassSgbd,0,5)=='sgbd_'){
				$sClassPath=_root::getConfigVar('path.lib').'sgbd/'.$this->sClassSgbd.'.php';
			}
			if(!file_exists($sClassPath)){
				...
				throw new Exception('Pas de driver '.$this->sClassSgbd.' ('.$sClassPath.')'."\n".$sListePdo."\n".$sListeAutre);
			}
		}

		$this->oSgbd=call_user_func( array($this->sClassSgbd,'getInstance'),$this->sConfig);

		if($bFirst){
			$this->oSgbd->setConfig($tClassSgbd);
		}
