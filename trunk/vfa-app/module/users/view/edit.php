		...
		<div class="panel-body">
			<div class="alert alert-warning clearfix">
				<p><?php echo plugin_validation::show($this->tMessage, 'token')?>
					<a class="btn btn-sm btn-warning pull-right" href="<?php echo $this->getLink('users::index') ?>">Fermer</a></p>
			</div>		
		</div>		
		<?php else:?>
		<div class="panel-body">
			<div class="row">
				...
				<div class="col-sm-6">
					<div class="<?php echo plugin_validation::addClassError('form-group', $this->tMessage, 'email')?>">
						...
						<input class="form-control" type="text" id="inputEmail" name="email" value="<?php echo $this->oUser->email ?>" />
			...
			<div class="<?php echo plugin_validation::addClassError('form-group', $this->tMessage, 'vote')?>">
				<label for="inputVote">Electeur</label>
				<div class="controls">
					<input type="checkbox" id="inputVote" name="vote" <?php if ($this->oUser->vote): echo 'checked'; endif; ?> />
					<span class="help-block"><?php echo plugin_validation::show($this->tMessage, 'vote')?></span>
				</div>
			</div>
			*/ ?>
			<div class="<?php echo plugin_validation::addClassError('form-group', $this->tMessage, 'user_groups')?>">
				<label for="inputUserGroups">Groupes 
					<span class="btn btn-xs btn-link" data-rel="tooltip" data-original-title="Choisir un ou plusieurs groupes pour l'utilisateur.">
			...
			<div class="<?php echo plugin_validation::addClassError('form-group', $this->tMessage, 'user_roles')?>">
				<label for="inputUserRoles">Rôles 
					<span class="btn btn-xs btn-link" data-rel="tooltip" data-original-title="Choisir un ou plusieurs rôles pour l'utilisateur.">
						<i class="glyphicon glyphicon-info-sign"></i>
					</span>
				</label>
					<select id="inputUserRoles" class="form-control" name="user_roles[]" class="input-xxlarge" size="13" multiple>
		...
		<div class="panel-footer clearfix">
			<div class="pull-right">
				<?php if(trim($this->oUser->user_id)==false):?>
					<a class="btn btn-default" href="<?php echo $this->getLink('users::list') ?>"><i class="glyphicon glyphicon-remove with-text"></i>Annuler</a>
				<?php else:?>
					<a class="btn btn-default" href="<?php echo $this->getLink('users::read',array('id'=>$this->oUser->user_id)) ?>"><i class="glyphicon glyphicon-remove with-text"></i>Annuler</a>
				<?php endif;?>
