...
class _layout{
	...
	public function __construct($sLayout=null){
		...
		if($sLayout==null){
			throw new Exception('layout non definit dans le constructeur :(');
		}
	...
	public function __get($sVar){
		if(!array_key_exists($sVar,$this->tVar)){
			/*LOG*/_root::getLog()->error('Variable '.$sVar.' inexistante dans le layout '.$this->sLayout);
			throw new Exception('Variable '.$sVar.' inexistante dans le layout '.$this->sLayout);
		}else{
	...
	public function addModule($sPlace,$sAppel){
		...
		$oModule=new $sModule;
		$this->tContent[$sPlace][]=call_user_func( array($oModule,'_'.$sAction));
	}
	/**
	...
	public function show(){
		/*LOG*/_root::getLog()->info('-layout: affichage ['._root::getConfigVar('path.layout').$this->sLayout.'.php]');
		if(!file_exists( _root::getConfigVar('path.layout').$this->sLayout.'.php')){
			$tErreur=array(
					'layout '.$this->sLayout.' introuvable dans '._root::getConfigVar('path.layout'),
					_root::getConfigVar('path.layout').$this->sLayout.'.php introuvable',
			);
			throw new Exception(implode("\n",$tErreur));
		}
		include(_root::getConfigVar('path.layout').$this->sLayout.'.php');
	}
	/**
	...
	public function load($sPlace){
		...
		if(!isset($this->tContent[$sPlace])) return null;
		$sLoad='';
		foreach($this->tContent[$sPlace] as $oTpl){
			$sLoad.=$oTpl->show();
		}
		return $sLoad;
	}
