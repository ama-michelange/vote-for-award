...
SELECT *
FROM vfa_votes
WHERE modified >= DATE("2014-11-08");
...
--  vfa_authorizations
CREATE INDEX authorization_role_module_action ON vfa_authorizations (role_id, module(10), action(10));

--  vfa_awards
CREATE INDEX award_year_name_type ON vfa_awards (year, name(15), type(3));
CREATE INDEX award_type_start_date_name ON vfa_awards (type(3), start_date, name(15));
CREATE INDEX award_selection ON vfa_awards (selection_id);
CREATE INDEX award_end_date ON vfa_awards (end_date);

--  vfa_docs
CREATE INDEX doc_order_title_number ON vfa_docs (order_title(15), number(5));
CREATE INDEX doc_date_legal_order_title_number ON vfa_docs (date_legal, order_title(15), number(5));
...
-- vfa_group_awards
ALTER TABLE vfa_group_awards ADD id INT NOT NULL AUTO_INCREMENT PRIMARY KEY
FIRST;
...
ALTER TABLE vfa_selection_titles ADD id INT NOT NULL AUTO_INCREMENT PRIMARY KEY
FIRST;
...
-- vfa_title_docs
ALTER TABLE vfa_title_docs ADD id INT NOT NULL AUTO_INCREMENT PRIMARY KEY
FIRST;
...
-- vfa_user_awards
ALTER TABLE vfa_user_awards ADD id INT NOT NULL AUTO_INCREMENT PRIMARY KEY
FIRST;
...
-- vfa_user_groups
ALTER TABLE vfa_user_groups ADD id INT NOT NULL AUTO_INCREMENT PRIMARY KEY
FIRST;
...
-- vfa_user_roles
ALTER TABLE vfa_user_roles ADD id INT NOT NULL AUTO_INCREMENT PRIMARY KEY
FIRST;
...
WHERE (vfa_user_groups.user_id = vfa_users.user_id) AND (vfa_user_groups.group_id = 3);
...
SELECT *
FROM vfa_titles
WHERE title = 'Blue Note' AND numbers = '#1';
