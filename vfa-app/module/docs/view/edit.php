			...
			<div class="panel-body">
				...
				<div class="<?php echo plugin_validation::addClassError('form-group', $this->tMessage, 'number') ?>">
					...
					<span class="btn btn-xs btn-link" data-rel="tooltip"
								data-original-title="Si l'album fait partie d'une série, numéro dans la série">
						<i class="glyphicon glyphicon-info-sign"></i>
					</span>
					</label>
					<input class="form-control" type="text" id="inputNumber" name="number"
								 value="<?php echo $this->oDoc->number ?>"/>
				...
				<div class="form-group">
					...
					<input class="form-control" type="text" id="inputProperTitle" name="proper_title"
								 value="<?php echo $this->oDoc->proper_title ?>"/>
					<span
						class="help-block"><?php if ($this->tMessage and isset($this->tMessage['proper_title'])): echo implode(',', $this->tMessage['proper_title']); endif; ?></span>
				</div>
				<div class="form-group">
					<label for="inputImage">Adresse de l'image
					<span class="btn btn-xs btn-link" data-rel="tooltip"
								data-original-title="Adresse WEB complète (URL) de l'image de l'album">
						<i class="glyphicon glyphicon-info-sign"></i>
					</span>
					</label>
					<input class="form-control" type="text" id="inputImage" name="image"
								 value="<?php echo $this->oDoc->image ?>"/>
					<span
						class="help-block"><?php if ($this->tMessage and isset($this->tMessage['image'])): echo implode(',', $this->tMessage['image']); endif; ?></span>
				</div>
				<div class="form-group">
					<label for="inputUrl">Adresse du résumé
					<span class="btn btn-xs btn-link" data-rel="tooltip"
								data-original-title="Adresse WEB complète (URL) d'accès au résumé de l'album">
						<i class="glyphicon glyphicon-info-sign"></i>
					</span>
					</label>
					<input class="form-control" type="text" id="inputUrl" name="url" value="<?php echo $this->oDoc->url ?>"/>
					<span
						class="help-block"><?php if ($this->tMessage and isset($this->tMessage['url'])): echo implode(',', $this->tMessage['url']); endif; ?></span>
				</div>
			</div>
				...
				<div class="pull-right">
					...
					<?php else: ?>
						<a class="btn btn-default"
							 href="<?php echo $this->getLink('docs::read', array('id' => $this->oDoc->doc_id)) ?>">
							<i class="glyphicon glyphicon-remove with-text"></i>Annuler
						</a>
					<?php endif; ?>
				</div>
