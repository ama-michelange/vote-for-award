...
//		$this->execute('DELETE FROM vfa_award_selections WHERE award_id=?', $pIdAward);
     ...
     * @return row_award[]
     */
    public function findAllAwardsByYear($year)
    {
        $sql = 'SELECT * FROM ' . $this->sTable . ' WHERE year=? AND type=? ORDER BY LEFT(name,2), public DESC';
        return $this->findMany($sql, $year, plugin_vfa::TYPE_AWARD_READER);
    }
