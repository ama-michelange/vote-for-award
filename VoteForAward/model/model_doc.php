...
class model_doc extends abstract_model{
	...
	public function findAllRecent(){
		...
		return $this->findMany('SELECT * FROM '.$this->sTable
				.' WHERE date_legal >= DATE(\''.$date->toString().'\') ORDER BY order_title,LENGTH(number),number,proper_title');
	}
	public function findAllByTitleId($pTitleId) {
		$sql = 'SELECT * FROM vfa_docs, vfa_title_docs '
				.'WHERE (vfa_title_docs.doc_id = vfa_docs.doc_id) '
						.'AND (vfa_title_docs.title_id = ?) '
								.'ORDER BY vfa_docs.order_title,LENGTH(number),number,proper_title';
