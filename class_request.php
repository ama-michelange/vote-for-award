...
class _request{
	...
	public function __construct(){
		...
		if($this->sModule==null) $this->sModule=_root::getConfigVar('navigation.module.default');

		$this->sAction=_root::getConfigVar('navigation.action.default',null);
		if($this->sAction==null) $this->sAction=_root::getConfigVar('navigation.action.default');
		...
		if (array_key_exists($sVar,$this->tVar)){
			if ( (int)_root::getConfigVar('security.xss.enabled')==1) {
				if ( 1 == (int)_root::getConfigVar('security.xss.onlyspecialchars')) {
					...
					$newVal = _root::nullbyteprotect(htmlspecialchars($this->tVar[$sVar],ENT_QUOTES,_root::getConfigVar('encodage.charset')));
					return $newVal;
				}
				else {
					if(is_array($this->tVar[$sVar]) ){
					...
					return _root::nullbyteprotect(htmlentities($this->tVar[$sVar],ENT_QUOTES,_root::getConfigVar('encodage.charset')));
				}
			}
			else {
