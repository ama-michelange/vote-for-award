...
class _dir
{
	 ...
	 *          tableau des extensions a prendre
	 * @param array $tExclusion
	 *          tableau des extensions a exclure
	 * @param string $sType
	 *          dir|file pour filtrer si besoin que les fichiers/repertoires
	 * @param boolen $bWithHidden
	 *          boolean avec fichier cache ou non
	...
	public function getList($tInclusion = null, $tExclusion = null, $bWithHidden = false)
	{
		...
		while (false !== ($sFile = readDir($open))) {
			...
			if ($bWithHidden == false and $sFile[0] == '.') {
				...
				$tFile[] = $oElement;
			} elseif (($tInclusion == null or in_array(end($tDetailFile), $tInclusion)) and
	...
	public function getListFile($tInclusion = null, $tExclusion = null, $bWithHidden = false)
	{
		...
		while (false !== ($sFile = readDir($open))) {
			...
			if ($bWithHidden == false and $sFile[0] == '.') {
				continue;
			} elseif ($bIsDir == false and ($tInclusion == null or in_array(end($tDetailFile), $tInclusion)) and
				($tExclusion == null or !in_array(end($tDetailFile), $tExclusion))
			) {
				$oElement = new _file($this->_sAdresse . '/' . $sFile);
				$tFile[] = $oElement;
