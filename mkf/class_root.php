...
class _root{
	...
	static protected $oCache;
	static protected $oTools;
	static protected $oAuth;
	static protected $oUrlrewriting;
	static protected $oLog;

	static protected $tConfigFilename;
	static protected $tRequestTab;
	static protected $tAutoload;

	static protected $tConfigVar;
	static protected $tGlobalVar;
	static protected $tObject;

	static protected $oACL;

	static protected $bSessionStarted=0;
	 ...
	 */
	public static function addConf($sConfig){
		self::$tConfigFilename[]=$sConfig;
	}
	...
	public static function startSession(){
		if(self::$bSessionStarted){
			...
			// Version php >= 5.2.0
			// session_set_cookie_params((int)self::getConfigVar('auth.session.cookie_lifetime',0),_root::getConfigVar('auth.session.cookie_path',null),_root::getConfigVar('auth.session.cookie_domain',null),$bSecure,$bHttpOnly);
			// Version php < 5.2.0
			session_set_cookie_params((int)self::getConfigVar('auth.session.cookie_lifetime',0),_root::getConfigVar('auth.session.cookie_path',null),_root::getConfigVar('auth.session.cookie_domain',null),$bSecure);
		}
		session_start();

		self::$bSessionStarted=1;
	}
